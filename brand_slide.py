...
Render a text banner that slides vertically across an ultra-wide canvas (default
7080×108 px). Optional side logos can fade in/out and remain static once in
place. All inputs are local; no Brandfetch integration.

Example:
    python brand_slide.py --text "MERCURIAL VAPOR" --font-path "Futura.ttf" \
      --bg-color "#000000" --font-color "#FFFFFF" --side-logo nike_logo.png \
      --text-fade-in 0.5 --text-fade-out 0.5

Dependencies:
    pip install pillow moviepy cairosvg (optional for SVG logos)
"""
...
def main() -> None:
    ...
    if not args.text:
        raise SystemExit("Bitte --text angeben oder im JSON setzen.")
    ...
    text_img = render_text_autosize(args.text, args.font_path, text_rgb, target_h)
...
if __name__ == "__main__":
    try:
        import json  # lazy import for config handling
    except Exception:
        json = None
    try:
        if json is None:
