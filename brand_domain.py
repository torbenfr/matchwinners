...
Render eine Domain (z. B. "www.pluto.tv") als Werbebotschaft mit Slide-in/Hold/Slide-out.
- Domain kann aus Brandfetch geholt werden (identifier = Marke/Domain) ODER lokal via --domain gesetzt werden.
- Schrift wird wie im vorherigen Script geladen (Brandfetch-Font bevorzugt; lokal via --font-path).
- Autosizing: Schrift-Bounding-Box-Höhe <= height * height_factor - height_padding (Standard 98 %).
- Korrekte Baseline (kein Clipping), kein Resampling → scharfe Darstellung.
- Optional: pix_fmt/crf zur Kontrolle der H.264-Ausgabequalität.
...
def load_font(font_path: Optional[str], size: int) -> ImageFont.FreeTypeFont:
    ...
    raise SystemExit("Konnte keine skalierbare Schrift laden. "
                     f"Versuche --font-path auf eine .ttf/.otf/.ttc zu setzen.")
...
def main():
    ...
    parser.add_argument("--identifier", required=True, help="'local' oder Brand-Identifier (Domain/Name)")
    parser.add_argument("--api-key", required=False, help="Brandfetch API-Key (wenn identifier != local)")
    ...
    parser.add_argument("--domain", type=str, default=None,
                        help="Domain-Text, der gerendert wird (z. B. 'www.pluto.tv'). Wenn nicht gesetzt: Brandfetch-Domain oder identifier.")

    # Farben/Fonts
    parser.add_argument("--font-path", type=str, default=None, help="Pfad zu .ttf/.otf/.ttc (lokal/Fallback)")
    parser.add_argument("--font-color", type=str, default=None, help="Hex #RRGGBB")
    parser.add_argument("--bg-override", type=str, default=None, help="Hintergrundfarbe überschreiben (Hex)")
    ...
    # Autosize
    parser.add_argument("--height-factor", type=float, default=0.98, help="Anteil der Canvas-Höhe für Text")
    parser.add_argument("--height-padding", type=int, default=0, help="Pixel-Luft, die zusätzlich abgezogen wird")

    # Encoding
    parser.add_argument("--pix-fmt", type=str, default="yuv420p", help="FFmpeg pix_fmt (z. B. yuv420p, yuv444p)")
    parser.add_argument("--crf", type=int, default=18, help="x264 CRF (niedriger = bessere Qualität)")

    args = parser.parse_args()

    # Domain-Text bestimmen
    domain_text: Optional[str] = args.domain
    if args.identifier.lower() == "local":
        if not domain_text:
            raise SystemExit("Lokaler Modus: Bitte --domain angeben (z. B. --domain 'www.example.com').")
        ...
        if not args.api_key:
            raise SystemExit("Brandfetch-Modus benötigt --api-key.")
        ...
        if not domain_text:
            # Wenn Brandfetch keine Domain liefert, nutze identifier
            domain_text = brand.domain or str(args.identifier)

    # Textbild erzeugen (Autosize 98% – genauer: height_factor)
    target_h = max(1, int(round(args.height * args.height_factor)) - max(0, args.height_padding))
    text_img = render_text_autosize(domain_text, font_path, text_rgb, target_h)
